...
class ParallelFaces(QMainWindow, Ui_MainWindow):
    ...
    def fig1(self):
        ...
        Xmin = -5E-1                                # Interval of variation for the
        ...
        if self.cbPhi.isChecked() == True:          # Relflection shift (pi)
            cbPhi = 1
        else:
            cbPhi = 0
        diffmarch1 = 2. * ndx * thick * \
            np.cos(angle_refract) + cbPhi * Lambda1 / 2.  # OPD 1
        I_B = 0.5 * (1. + np.cos(2. * pi * diffmarch1 / Lambda1))  # I(OPD1)
        diffmarch2 = 2. * ndx * thick * \
            np.cos(angle_refract) + cbPhi * Lambda2 / 2.  # OPD 2
        ...
        mpl1d.ax.set_xlabel(r'$r (rad)$', fontsize=12, fontweight='bold')
        mpl1d.ax.set_ylabel(r'$I(\theta)$', fontsize=12, fontweight='bold')
...
#        plt.tight_layout(rect=(0.,0.04,1,1))
    ...
    def on_pushButton_Lambda3_clicked(self):
        self.figRed()
        plt.get_current_fig_manager().window.setGeometry(300, 0, 300, 300)
        plt.show(self.figR)
